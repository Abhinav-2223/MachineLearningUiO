...
\begin{case}[the bias-variance decomposition, owned]
...
\textbf{Goal:} a $k$-fold loop you wrote yourself, validated against
\texttt{scikit-learn}, then used for real model selection (notebook, Case 2).
\begin{enumerate}
  ...
  \item Check yourself against \texttt{cross\_val\_score} with a
        \texttt{Pipeline}: the two curves must coincide to plotting accuracy.
        If they do not, the usual culprit is scaling fitted on the full data.
...
noise floor).  For step 4: CV picks a degree in the same broad 5--9 window as
the bootstrap decomposition -- two independent methods, one answer.
\end{block}
\end{case}
...
One line on a piece of paper (or the digital board): the muddiest point of
the week -- what should Monday's first ten minutes revisit?
\end{block}
\end{frame}
